...
module axi_systolic_wrapper #(
    ...
    // Final adder isolated by 2 registers.
    ...
    always @(posedge aclk) begin
        if (!aresetn) begin
            ...
            valid_pipe <= {valid_pipe[70:0], pipe_start};
            last_pipe  <= {last_pipe[70:0], (pipe_start && s_axis_a_tlast)};
        end
    end

    assign m_axis_c_tvalid = valid_pipe[71];
    assign m_axis_c_tdata = packed_c_out;
    assign m_axis_c_tlast = last_pipe[71];
